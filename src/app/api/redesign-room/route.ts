...
export async function POST(request: NextRequest) {
  return withCredit(request, async (request: NextRequest) => {
  try {
    const body = (await request.json()) as RedesignRequestBody;
    const { imageData, mimeType, roomType, themes, variantSeed, intensity } =
      body;
    ...
    for (let i = 0; i < Math.min(maxConcurrent, limitedThemes.length); i += 1) {
